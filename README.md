...
- [x] Memo 系统通知：其他人创建、更新、删除、调整相关人时触发操作系统通知
- [x] 通用通知平台前端接入：通知中心可拉取服务端历史通知，接收 WebSocket 通用通知并触发系统弹窗

### 开发中 🚧
- [ ] 企业级通知平台外部系统专用鉴权：appKey/appSecret、签名、限流和来源系统权限
- [ ] 剪贴板
...
- 下拉选择使用 Radix Select 封装，避免 Windows/macOS 原生 select 样式不一致
...
- 事件发起人不是当前用户时，`memo.created`、`memo.updated`、`memo.deleted`、`memo.related.updated` 写入通知中心，并通过 Tauri notification 插件发送操作系统通知
- Memo 通知优先展示 Memo 标题、更新者和内容摘要；这些字段由后端实时事件直接携带，前端不为弹窗额外请求详情
- `group.*` 事件只刷新分组，不生成通知
- 置顶和收藏是个人状态，由接口返回值在操作者本地更新，不广播实时事件，也不生成通知

## 通知平台规划

- Memo 实时通知仍作为轻量协作事件处理：依赖 WebSocket 事件、通知中心本地状态和操作系统通知，不写入通用通知表
- 通用通知平台已接入最小闭环：后端可创建通知并落库，桌面端通过 WebSocket 接收新通知，通过接口查询历史和同步已读状态
- 通知中心测试按钮会创建一条发给当前用户的服务端通用通知，用于单机验证落库、WebSocket、通知中心和系统弹窗
- 后续企业级增强：外部系统专用 appKey/appSecret 鉴权、签名校验、限流、来源系统权限、部门/角色接收人和服务端清理策略
