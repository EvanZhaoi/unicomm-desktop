...
### 开发中 🚧
- [ ] 企业级通知平台：后端通用通知接口、数据库持久化、外部系统接入
- [ ] 剪贴板
...
- 当前阶段先实现 Memo 实时通知：依赖 WebSocket 事件、通知中心内存列表和操作系统通知，不进入数据库持久化
- 后续企业级通知平台应独立于 Memo 模块：由后端提供通用通知创建接口，外部系统可提交标题、内容、来源系统、接收人、级别、跳转目标等字段
- 通用通知需要服务端落库，桌面端通过 WebSocket 接收新通知，同时可通过通知中心查询历史、未读数、已读状态和清理记录
